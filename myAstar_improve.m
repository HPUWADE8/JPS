...
% 填充起点 绿色 根据四个点去填充 
fill([start_node(1)-1,start_node(1)-1,start_node(1),start_node(1)],...
     [start_node(2)-1,start_node(2),start_node(2),start_node(2)-1],'g');

% 填充终点 红色
fill([target_node(1)-1,target_node(1)-1,target_node(1),target_node(1)],...
     [target_node(2)-1,target_node(2),target_node(2),target_node(2)-1],'r');

% 填充障碍物
for i=1:size(obstacle,1)
    temp = obstacle(i,:);
   fill([temp(1)-1,temp(1)-1,temp(1),temp(1)],...
     [temp(2)-1,temp(2),temp(2),temp(2)-1],'k');
 ...
 if isequal(parent_node,target_node)
    ...
    flag = 0;
       for i = 1:(size(openlist,1))
        temp = openlist(i,:);
        fill([temp(1)-1,temp(1)-1,temp(1),temp(1)],...
         [temp(2)-1,temp(2),temp(2),temp(2)-1],[0.5, 0.7, 1]);
    end
  
    for i = 1:(size(closelist,1)-1)
        temp = closelist(i+1,:);
        fill([temp(1)-1,temp(1)-1,temp(1),temp(1)],...
         [temp(2)-1,temp(2),temp(2),temp(2)-1],[0.7, 0.9, 1]);
    end
    fill([start_node(1)-1,start_node(1)-1,start_node(1),start_node(1)],...
     [start_node(2)-1,start_node(2),start_node(2),start_node(2)-1],'g');
